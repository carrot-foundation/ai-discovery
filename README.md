...
`llms.txt` generators, AI-aware `robots.txt` helpers, and a Markdown mirror
route for framework sites.
...
This package is under Phase 0 review. Release-triggering merges to `main`
publish automatically after CI succeeds, so only merge them after Cristiano has
reviewed the package PR and tarball.
